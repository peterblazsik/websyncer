...
import type { BatchGenerationProgress } from '../../types/batch';
...
export const BatchProgressModal: React.FC<BatchProgressModalProps> = ({
    ...
    // Truncate current prompt for display
    const truncatedCurrent =
        progress.current.length > 60
            ? progress.current.slice(0, 57) + '...'
            : progress.current;
            ...
            <div className="bg-brand-card border border-brand-border rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl">
                ...
                <div className="flex items-center justify-between mb-6">
                    <h3 className="text-lg font-bold text-white">
                        {isComplete ? 'Batch Complete' : 'Generating Images...'}
                    </h3>
                    {isComplete && (
                        <button
                            onClick={onClose}
