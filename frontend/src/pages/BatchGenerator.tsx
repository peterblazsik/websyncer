...
export function BatchGenerator() {
  // Input state
  const [inputMode, setInputMode] = usePersistedState<BatchInputMode>(
    "batch-input-mode",
    "quick",
  );
  const [quickText, setQuickText] = useState("");
  const [globalAspectRatio, setGlobalAspectRatio] =
    usePersistedState<BatchAspectRatio>(
      "batch-aspect-ratio",
      DEFAULT_ASPECT_RATIO,
    );
  ...
  // Target directory
  const [targetDir, setTargetDir] = usePersistedState("batch-target-dir", "");
  ...
  const generateAll = async () => {
    // Validation
    if (inputMode === "quick" && quickText.trim().length === 0) {
      toast.error("Enter at least one prompt before generating.");
      return;
    }
    if (
      inputMode === "structured" &&
      items.filter((i) => i.prompt.trim()).length === 0
    ) {
  ...
  const loadPreset = (json: any) => {
    try {
      if (!json.items || !Array.isArray(json.items)) {
        toast.error('Invalid preset format: missing "items" array');
        return;
      }
      ...
      const loadedItems: BatchPromptItem[] = json.items.map(
        (item: any, index: number) => ({
          ...
          aspectRatio: (item.aspectRatio ||
            json.aspectRatio ||
            "1:1") as BatchAspectRatio,
          filename:
            item.filename || `batch-${String(index + 1).padStart(3, "0")}.webp`,
          status: "pending" as const,
        }),
      );
  ...
  const loadPresetFromUrl = async (url: string) => {
    try {
      ...
      toast.error("Failed to load preset from URL");
  ...
  const currentItemCount =
    inputMode === "quick"
      ? promptCount
      : items.filter((i) => i.prompt.trim()).length;
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start gap-2 p-3 bg-white/5 border border-brand-border rounded-lg">
          ...
          <p className="text-xs text-brand-muted">
            Aspect ratio is suggested in the prompt text. Gemini 3 Pro doesn't
            have a native aspect ratio parameter, so actual output dimensions
            may vary slightly.
          </p>
        </div>
