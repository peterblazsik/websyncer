...
import {
  OutputOptions,
  type OutputMode,
} from "../components/branding/OutputOptions";
...
export function BrandingGenerator() {
  ...
  const [autoSave, setAutoSave] = usePersistedState("branding-auto-save", true);
  const [watermarkEnabled, setWatermarkEnabled] = usePersistedState(
    "branding-watermark-enabled",
    false,
  );
  const [watermarkPosition, setWatermarkPosition] =
    usePersistedState<WatermarkPosition>(
  ...
  const generateTimestampedPath = (
    spec: ReturnType<typeof getImageById>,
  ): string => {
    if (!spec) return "";
    const now = new Date();
    const timestamp = [
  ...
  const downloadAllAsZip = async () => {
    ...
    if (images.size === 0) {
      toast.error("No images to download. Generate some images first.");
      return;
    }
    ...
    for (const [imageId, imageUrl] of images) {
      ...
      try {
        ...
        const outputPath =
          outputMode === "save" ? spec.outputPath : `${imageId}.webp`;
  ...
  const downloadCategoryAsZip = async () => {
    ...
    const available = categoryImages.filter((img) =>
      generatedImages.has(img.id),
    );

    if (available.length === 0) {
      toast.error("No images generated in this category yet.");
      return;
    }
    ...
    for (const spec of available) {
      ...
      try {
        ...
        const outputPath =
          outputMode === "save" ? spec.outputPath : `${spec.id}.webp`;
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="section-subtitle max-w-2xl">
              Generate premium brand assets for the ARTIN platform.{" "}
              {TOTAL_IMAGE_COUNT} images total.
            </p>
          </div>
          <div className="flex items-center gap-4">
            ...
            >
              <HardDrive size={14} />
              <span>
                {localSaverConnected ? "Auto-save ON" : "Auto-save OFF"}
              </span>
              {localSaverConnected && (
                <button
