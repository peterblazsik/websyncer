...
function getWhitelistIPs(env: { WHITELIST_IPS?: string }): string[] {
    ...
    return env.WHITELIST_IPS.split(',').map(ip => ip.trim()).filter(Boolean);
}
...
app.get('/api/rate-limit-status', async (c) => {
    ...
    const fingerprint = await createFingerprint(ip, userAgent, acceptLanguage, acceptEncoding);
...
app.post('/api/generate', async (c) => {
    try {
        ...
        Promise.all([
            c.env.RATE_LIMIT.put(
                shortTermKey,
                (currentShortTerm + 1).toString(),
  ...
  `.trim();
        ...
        if (data.predictions && data.predictions.length > 0 && data.predictions[0].bytesBase64Encoded) {
            ...
            return c.json({
                ...
                imageUrl: imageUrl,
                rateLimit: {
                    shortTermRemaining: limits.SHORT_TERM.MAX_REQUESTS - currentShortTerm - 1,
...
app.post('/api/branding/generate', async (c) => {
    try {
        ...
        const apiAspectRatio = supportedRatios.includes(aspectRatio) ? aspectRatio : '1:1';
        ...
        if (Object.keys(data).length === 0) {
            throw new Error("Generation blocked by safety filter. Try modifying the prompt.");
        }
